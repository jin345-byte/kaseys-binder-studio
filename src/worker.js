...
function jwtPart(v){return JSON.parse(new TextDecoder().decode(b64u(v)))}
async function sha(v){const d=new Uint8Array(await crypto.subtle.digest('SHA-256',new TextEncoder().encode(v)));return[...d].map(x=>x.toString(16).padStart(2,'0')).join('')}
async function jwks(){const c=caches.default,k=new Request('https://kbs.internal/google-jwks');let r=await c.match(k);if(!r){const u=await fetch('https://www.googleapis.com/oauth2/v3/certs');if(!u.ok)throw new Error('Could not load Google signing keys');r=new Response(await u.text(),{headers:{'content-type':'application/json','cache-control':'public, max-age=1800'}});await c.put(k,r.clone())}return r.json()}
