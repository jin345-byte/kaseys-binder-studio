...
const previewJson=(data,status=200,extra={})=>new Response(JSON.stringify(data),{status,headers:{
  'content-type':'application/json; charset=utf-8',
  'cache-control':'no-store, no-cache, must-revalidate, max-age=0',
  'x-kbs-preview-read-only':'1',
  ...extra
}});
...
async function readOnlySessionUser(request,env){
  ...
  const token=cookies(request).kbs_session;
...
async function readOnlyPreviewApi(request,env){
  ...
  if(path==='/api/config')return previewJson({
    googleClientId:'',databaseReady:Boolean(env.DB),authReady:false,
    productionAuthConfigured:Boolean(env.DB&&env.GOOGLE_CLIENT_ID),syncVersion:2,
    previewReadOnly:true,previewEnvironment:PREVIEW_ENVIRONMENT
  });
  ...
  if(path==='/api/sync'){
    ...
    const user=await readOnlySessionUser(request,env);
    if(!user)return previewJson({error:'Sign in required.',previewReadOnly:true},401);
...
function reversedTwoPartName(raw){
  const clean=tidyQuery(raw),parts=clean.split(' ').filter(Boolean);
...
async function callArtV2(request,env,ctx,query,pid){
  ...
  u.pathname='/api/art-feed-v2';u.search='';u.searchParams.set('tag',query);u.searchParams.set('pid',String(pid));
...
async function characterArtworkFeed(request,env,ctx){
  ...
  if(!plan.length)return previewJson({results:[],done:true,error:'Invalid artwork query'},400);
  const fetched=[];
  ...
  return previewJson({
    results:merged,pid,nextPid:pid+1,done:merged.length===0&&!hadError,broad:true,characterSearch:true,
    queryPlan:plan,zerochanConfigured,sourceCounts:sources,
    error:merged.length?'':(hadError?'One or more artwork sources are temporarily unavailable':'')
  },200,{
    'x-kbs-art-broad':'1','x-kbs-character-search':'1','x-kbs-zerochan-configured':zerochanConfigured?'1':'0'
  });
}
...
export default{
  async fetch(request,env,ctx){
    ...
    if(url.pathname.startsWith('/api/')){
      ...
      else if(url.pathname==='/api/art-feed-v3')response=await characterArtworkFeed(request,safeStagingEnv(env),ctx);
      else if(SAFE_STAGING_GET_APIS.has(url.pathname))response=await stagingWorker.fetch(request,safeStagingEnv(env),ctx);
      else response=await readOnlyPreviewApi(request,env);
    }else if(env.ASSETS){
      response=await env.ASSETS.fetch(request);
    }else{
      response=new Response('Binder Studio preview assets binding is unavailable.',{status:503});
    }

    const headers=new Headers(response.headers);
    headers.set('x-kbs-production-preview',PREVIEW_BUILD);
    headers.set('x-kbs-preview-read-only','1');
    headers.set('x-kbs-preview-asset-isolation','1');
