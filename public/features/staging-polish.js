/* Binder Studio v4.0.4 staging polish — hover preview, wheel zoom, static brand, page rail, Art of Pokemon proxy */
(function(){
  ...
  function showPreview(img,e){if(!canHover()||!img)return;previewTarget=img;previewImg.src=img.currentSrc||img.src||img.dataset.fallbackSrc||'';preview.hidden=false;preview.classList.add('visible');placePreview(e.clientX,e.clientY)}
  function hidePreview(){previewTarget=null;preview.classList.remove('visible');preview.hidden=true}
  ...
  /* Brand lettering and page navigation are intentionally static in v4.0.4. */
  const pageNumbers=document.querySelector('#editorPageNumbers');
  function revealActivePage(){if(!pageNumbers)return;const active=pageNumbers.querySelector('.page-number.active,[aria-current="page"]');if(active)active.scrollIntoView({behavior:'auto',block:'center',inline:'nearest'})}
  if(pageNumbers){new MutationObserver(()=>requestAnimationFrame(revealActivePage)).observe(pageNumbers,{childList:true,subtree:true,attributes:true,attributeFilter:['class','aria-current']});pageNumbers.addEventListener('click',()=>setTimeout(revealActivePage,20));document.querySelector('#editorPrev')?.addEventListener('click',()=>setTimeout(revealActivePage,40));document.querySelector('#editorNext')?.addEventListener('click',()=>setTimeout(revealActivePage,40));setTimeout(revealActivePage,150)}
