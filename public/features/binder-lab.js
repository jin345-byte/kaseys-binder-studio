/* Kasey's Binder Studio v4.0.5 — binder overview, copy and reorder tools.
   Page-transition animation hooks were removed so page changes are immediate. */
const KBSBinderLab=(()=>{
  async function duplicatePage(pageId){
    ...
    const copy={...page,id:id('page'),order:at+1,title:`${page.title||`Page ${at+1}`} copy`,createdAt:Date.now(),updatedAt:Date.now(),state:JSON.parse(JSON.stringify(page.state||defaults))};
    await dbPut('pages',copy);await renderEditorPageNav();await renderBinderLibrary();toast('Page duplicated');
  }
  ...
  async function reorderPage(dragId,targetId){
    ...
    for(let i=0;i<pages.length;i++)if(pages[i].order!==i){pages[i].order=i;pages[i].updatedAt=Date.now();await dbPut('pages',pages[i])}
  ...
  renderBinderLibrary=async function(){const r=await coreRenderBinderLibrary.apply(this,arguments);enhanceOverview();return r};
