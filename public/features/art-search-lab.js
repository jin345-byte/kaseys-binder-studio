...
(function(){
  ...
  function normalizeFanRows(rows,raw,key){
  ...
  async function fetchFanCache(raw,signal){
    const key=cacheKey(raw);if(!key)return {rows:[],tag:booruTag(raw)};
    try{
      const r=await fetch('/art-cache/'+encodeURIComponent(key)+'.json',{signal,headers:{Accept:'application/json'},cache:'no-store'});
      if(r.status===404)return {rows:[],tag:booruTag(raw)};
      if(!r.ok)throw new Error('Artwork cache HTTP '+r.status);
      const payload=await r.json();
      const rows=Array.isArray(payload)?payload:(Array.isArray(payload?.results)?payload.results:[]);
      return {rows:normalizeFanRows(rows,raw,key),tag:String(payload?.tag||booruTag(raw)).trim()||booruTag(raw)};
    }catch(e){if(e?.name==='AbortError')throw e;console.warn('Fan-art cache load failed',key,e);return {rows:[],tag:booruTag(raw)};}
  }

  async function fetchFanPage(raw,tag,pid,signal){
    const q=String(tag||booruTag(raw)).trim();if(!q)return {rows:[],done:true,nextPid:pid+1};
    try{
      const r=await fetch(`/api/art-feed?tag=${encodeURIComponent(q)}&pid=${encodeURIComponent(pid)}`,{signal,headers:{Accept:'application/json'},cache:'no-store'});
      if(!r.ok)return {rows:[],done:false,nextPid:pid+1};
      const payload=await r.json();
      return {rows:normalizeFanRows(payload?.results||[],raw,cacheKey(raw)+'-live-'+pid),done:payload?.done===true,nextPid:Number(payload?.nextPid)||pid+1,error:payload?.error||''};
    }catch(e){if(e?.name==='AbortError')throw e;console.warn('Paged artwork feed failed',e);return {rows:[],done:false,nextPid:pid+1,error:String(e?.message||e)};}
  }
  ...
  function addResult(article){
    ...
    if(urlInput&&sizeInput&&addBtn){urlInput.value=a.url;sizeInput.value=size;addBtn.click();article.classList.add('added');setTimeout(()=>article.classList.remove('added'),700);if(typeof toast==='function')toast('Artwork added to Art Tray');}
  }
    ...
    try{
      if(!entry.loaded){
        ...
        if(entry.visibleFan<entry.fan.length){
          ...
          const page=await fetchFanPage(raw,entry.tag,entry.nextPid,controller.signal);
          if(serial!==requestSerial)return;
          entry.nextPid=page.nextPid;
          const before=entry.fan.length;
          entry.fan=dedupe([...entry.fan,...page.rows]);
          const added=entry.fan.length-before;
          entry.visibleFan=Math.min(entry.fan.length,entry.visibleFan+Math.max(PAGE_SIZE,added));
          if(page.done||(page.rows.length===0&&!page.error))entry.exhausted=true;
        }
      }
      const rows=dedupe([...entry.official,...entry.fan.slice(0,entry.visibleFan)]);
      cache.set(q,entry);render(rows,raw);
      const cachedRemaining=Math.max(0,entry.fan.length-entry.visibleFan);
      if(entry.exhausted&&cachedRemaining===0){
        status.textContent=`${rows.length} artwork results shown · end of available results`;
        moreBtn.disabled=true;
      }else{
        status.textContent=cachedRemaining?`${rows.length} artwork results shown · ${cachedRemaining} ready to load`:`${rows.length} artwork results shown · more can be fetched`;
